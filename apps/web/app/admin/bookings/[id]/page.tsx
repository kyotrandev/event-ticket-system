...
import { adminApi, bookingApi, ticketApi } from '@/lib/api';
    ...
    async function load() {
      try {
        const [bk, tix, summaryRes] = await Promise.all([
          ...
          adminApi.getBookings({ limit: 50 }).catch(() => ({ data: [] })),
        ]);
        setBooking(bk);
        setTickets(tix);
        ...
        if (summary) {
          ...
          setOrganizerLabel(`${summary.organizerName} · ${summary.organizerEmail}`);
        }
        setError(null);
      } catch (e) {
    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <Card className="border-2 rounded-2xl">
          ...
          <CardContent className="text-sm">
            <p className="font-bold">{organizerLabel ?? '—'}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="border-2 rounded-2xl">
        ...
        <CardContent className="space-y-2 text-sm">
          ...
          {event?.id && (
            <div className="flex justify-between">
              <span className="text-muted-foreground">Event hub</span>
              <Link href={`/admin/events/${event.id}`} className="text-primary font-bold hover:underline">
                Open
              </Link>
            </div>
          )}
      ...
      <AlertDialog open={showCancelDialog} onOpenChange={setShowCancelDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {canCancelPaid
                ? 'All tickets will be invalidated and the customer will receive a full refund.'
                : 'Reserved seats will be released.'}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
